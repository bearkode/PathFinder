...
PENode *PEFindJumpNode(PEGrid *aGrid, PENode *aEndNode, CGFloat aX, CGFloat aY, CGFloat aPX, CGFloat aPY)
{
    ...
    if (sNormalizedX != 0 && sNormalizedY != 0)
    {
        ...
        PENode *sJumpNodeX = PEFindJumpNode(aGrid, aEndNode, (aX + sNormalizedX), aY, aX, aY);
        PENode *sJumpNodeY = PEFindJumpNode(aGrid, aEndNode, aX, (aY + sNormalizedY), aX, aY);
...
- (NSMutableArray *)findPathWithStartPosition:(CGPoint)aStartPosition endPosition:(CGPoint)aEndPosition grid:(PEGrid *)aGrid
{
    ...
    while ((sNode = [mOpenList pop]))
    {
        ...
        {
            PENode *sNeighbor = sNeighbors[i];
            
            if (sNeighbor)
            {
                CGPoint sNeighborPoint = [sNeighbor position];
                PENode *sJumpNode      = PEFindJumpNode(mGrid, mEndNode, sNeighborPoint.x, sNeighborPoint.y, sNodePosition.x, sNodePosition.y);
